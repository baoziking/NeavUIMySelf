...
function frame:COMBAT_LOG_EVENT_UNFILTERED(timeStamp, subEvent, _, _, sourceName, sourceFlags, _, _, destName, _, destRaidFlags, spellID, _, _, extraSpellID)
	...
	if CombatLogGetCurrentEventInfo then
		timeStamp, subEvent, _, _, sourceName, sourceFlags, _, _, destName, _, destRaidFlags, spellID, _, _, extraSpellID = CombatLogGetCurrentEventInfo()
	end
	...
	if sourceName == GetPlayerName() then
		...
		self:AnnounceInterrupt(config.own, string.format('%s打断了%s%s的%s', name, GetRaidIconString(raidIcon, config.own), destName or '?', GetSpellLinkCached(extraSpellID)), cfg.soundOwn)
	elseif bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) ~= 0 then
		-- If own is off for this config, return
		if config.own == 'off' then return end

		-- Announce interrupt
		self:AnnounceInterrupt(config.own, string.format('我的%s打断了%s%s的%s', sourceName or '?', GetRaidIconString(raidIcon, config.own), destName or '?', GetSpellLinkCached(extraSpellID)), cfg.soundOwn)
	else
		...
		if (inRaid and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_RAID) ~= 0) or (inGroup and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_PARTY) ~= 0) then
			...
			self:AnnounceInterrupt(config.other, string.format('%s打断了%s%s的%s', name, GetRaidIconString(raidIcon, config.other), destName or '?', GetSpellLinkCached(extraSpellID)), cfg.soundOther)
		end
	end
end
